    ...
    try {
      ...
      const processedPageviews = batch
        ...
        .map(pv => {
          ...
          const receivedAt = DateTime.fromISO(pv.timestamp).toUTC().toISO();
          ...
          return {
            ...
            // Include the UTC offset so a non-UTC ClickHouse server does not
            // reinterpret the receive time in its own timezone.
