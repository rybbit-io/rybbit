...
export function SiteCards({ organizationId, sites, ...props }: SiteCardsProps) {
  ...
  if (
    LITE_DASHBOARD &&
    !unboundedHourlyChart &&
    !hasRangeTimes(time) &&
    !(previousTime && hasRangeTimes(previousTime))
  ) {
    return <BatchedSiteCards organizationId={organizationId} sites={sites} {...props} />;
  }
  return sites.map(site => <SiteCard key={site.siteId} {...site} {...props} />);
}

function BatchedSiteCards({ organizationId, sites, ...props }: SiteCardsProps) {
