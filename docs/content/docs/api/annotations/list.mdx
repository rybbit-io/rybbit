...
<TypeTable
  type={{
    start_date: {
      description: 'Only annotations that end on or after this date (YYYY-MM-DD, UTC)',
      type: 'string',
      required: false,
    },
    end_date: {
      description: 'Only annotations that start on or before this date (YYYY-MM-DD, UTC)',
      type: 'string',
      required: false,
    },
  }}
