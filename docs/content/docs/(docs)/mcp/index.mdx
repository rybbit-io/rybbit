...
| Tool                        | Returns                                                                                          |
| --------------------------- | ------------------------------------------------------------------------------------------------ |
| `get_search_console_status` | Whether Google Search Console is connected for the site, and which property                      |
| `get_search_console_data`   | Clicks, impressions, CTR, and average position grouped by query, page, country, device, or date  |

Search Console tools read the Google Search Console property connected to the site under **Site settings > Integrations** (Rybbit Cloud); no Google Cloud project or separate MCP server is needed. `get_search_console_data` groups by up to three dimensions (`query`, `page`, `country`, `device`, `date`), accepts ANDed filters such as `{"dimension": "page", "operator": "contains", "expression": "/pricing"}`, and returns 100 rows by default (up to 25,000, paged with `start_row`). Google's numbers lag about two days; pass `data_state: "all"` to include fresh, still-changing data.
...
- **404 "GSC not connected" from Search Console tools:** Connect Google Search Console for that site under Site settings > Integrations, then retry.
