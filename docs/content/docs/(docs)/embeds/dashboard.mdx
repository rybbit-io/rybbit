...
On Rybbit Cloud, dashboard embed mode also requires a paid plan. Sites on the free plan cannot activate embed mode even with a valid private link — `embed=true` is ignored, so the sidebar and Rybbit footer stay visible and the `theme` and `hideSidebar` parameters have no effect.
