...
The key is user-scoped: the MCP server can do what that user can do. Deleting the key revokes the connection.
...
<Callout type="warn">
  This version uses API-key authentication, not interactive OAuth. It works with MCP clients that support custom HTTP
  headers. A client that requires browser-based OAuth cannot connect yet.
</Callout>
...
To give an assistant a read-mostly view, connect it with an API key from a member-role user: management tools will be visible but every call requiring elevation will be refused.
