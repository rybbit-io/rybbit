...
2. **Exclude the script from optimization:**
   Some optimization plugins try to defer, delay, combine, or minify scripts. "Delay JavaScript execution" features in particular rebuild the script tag and drop its `data-*` attributes, which is why the snippets on this page pass the site ID in the URL (`?siteId=`) instead of a `data-site-id` attribute. If Rybbit still isn't working, exclude `app.rybbit.io` in these plugins:
   ...
   - **Perfmatters / FlyingPress**: Add `app.rybbit.io` to the delay-JS exclusions
