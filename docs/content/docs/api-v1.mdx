...
<Callout type="warn" title="API v1 Status: Beta">
**API v1 currently has limited data.** The tracking system writes to ClickHouse while API v1 reads from PostgreSQL. For production use with full historical data, use the [legacy API endpoints](/docs/api) which include automatic gap filling and access to all analytics data. API v1 is in beta and being prepared for full production rollout.
</Callout>
