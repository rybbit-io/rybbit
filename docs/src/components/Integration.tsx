...
const PlatformLogo = ({ name, icon: Icon, path }: { name: string; icon: ComponentType<IconProps>; path: string }) => {
  return (
    <Link
      href={path}
      className={cn(
        "inline-flex h-10 items-center gap-2.5 rounded-md border px-3.5 text-sm",
        "border-neutral-200 dark:border-neutral-800 text-neutral-700 dark:text-neutral-300",
        "transition-colors hover:border-neutral-400 hover:text-neutral-950 dark:hover:border-neutral-600 dark:hover:text-white"
      )}
    >
