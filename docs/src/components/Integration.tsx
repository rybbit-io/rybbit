...
const PlatformLogo = ({ name, icon: Icon, path }: { name: string; icon: ComponentType<IconProps>; path: string }) => {
  return (
    <Link
      href={path}
      className="group flex flex-col justify-center gap-3 rounded-lg p-4 transition-colors hover:bg-neutral-100/70 dark:hover:bg-neutral-900"
    >
      <Icon className="h-6 w-6 text-neutral-500 dark:text-neutral-400 transition-colors group-hover:text-neutral-900 dark:group-hover:text-white" />
      <span className="text-sm text-neutral-600 dark:text-neutral-400 transition-colors group-hover:text-neutral-900 dark:group-hover:text-white">
        {name}
      </span>
    </Link>
  );
