...
const PlatformLogo = ({ name, icon: Icon, path }: { name: string; icon: ComponentType<IconProps>; path: string }) => {
  return (
    <Link
      href={path}
      className={cn(
        "flex items-center gap-2.5 rounded-lg border border-neutral-200 bg-white/60 px-3.5 py-2.5 text-sm",
        "text-neutral-700 transition-colors hover:border-neutral-400 hover:text-neutral-900",
        "dark:border-neutral-800 dark:bg-neutral-900/40 dark:text-neutral-300 dark:hover:border-neutral-600 dark:hover:text-white"
      )}
    >
