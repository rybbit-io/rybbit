  ...
  return (
    <div className="mb-6 md:mb-8" onClick={() => trackAdEvent("github")}>
      <a
        ...
        className="inline-flex items-center gap-1.5 rounded-full border border-neutral-200 bg-white/70 px-3.5 py-1.5 text-xs font-medium text-neutral-700 backdrop-blur transition-colors hover:border-neutral-300 hover:text-neutral-900 dark:border-neutral-800 dark:bg-neutral-900/70 dark:text-neutral-300 dark:hover:border-neutral-700 dark:hover:text-white"
      >
        ...
        {(isLoading || starCount) && (
          <>
            <span className="mx-1 h-3.5 w-px bg-neutral-200 dark:bg-neutral-700" aria-hidden="true" />
            <span className="flex items-center gap-1 tabular-nums">
              <Star className="w-3 h-3 text-yellow-500 fill-current" />
              {isLoading ? (
                <span className="w-8 h-3.5 bg-neutral-200 dark:bg-neutral-700 rounded-sm animate-pulse"></span>
              ) : (
                <span>{starCount}</span>
              )}
            </span>
