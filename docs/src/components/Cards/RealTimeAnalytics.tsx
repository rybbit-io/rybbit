...
function EventCard({ event, index, isNew }: { event: Event; index: number; isNew?: boolean }) {
  ...
  return (
    <div
      className="absolute inset-x-3 rounded-md border border-neutral-200 bg-white dark:border-neutral-800 dark:bg-neutral-900 overflow-hidden p-2 flex flex-col transition-all duration-500"
      style={{
  ...
  return (
    ...
    >
      <div className={previewScreen}>
        <div className="flex items-center gap-2 border-b border-neutral-200 px-4 py-2.5 text-xs text-neutral-500 dark:border-neutral-800 dark:text-neutral-400">
          <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 motion-safe:animate-pulse" />
          {t("{count} visitors online", { count: String(onlineCount) })}
        </div>
        <div className="relative p-3" style={{ height: "280px" }}>
